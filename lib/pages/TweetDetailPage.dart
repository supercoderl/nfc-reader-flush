...
class TweetDetailPage extends StatefulWidget {
  ...
  const TweetDetailPage({required Key key, required this.tweetData})
      : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return TweetDetailPageState(
        tweetData: tweetData, key: ValueKey('web-page-$tweetData'));
  }
}
...
class TweetDetailPageState extends State<TweetDetailPage> {
  ...
  TextStyle subtitleStyle =
      const TextStyle(fontSize: 12.0, color: Color(0xFFB5BDC0));
  TextStyle contentStyle = const TextStyle(fontSize: 15.0, color: Colors.black);
  num curPage = 1;
  final ScrollController _controller = ScrollController();
  final TextEditingController _inputController = TextEditingController();

  TweetDetailPageState({required Key key, required this.tweetData});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          title: const Text("动弹详情", style: TextStyle(color: Colors.white)),
                    ...
                    children: <Widget>[
                      ...
                      InkWell(
                        child: Container(
                          ...
                          child: const Text(
                            "发送",
                            style: TextStyle(color: Color(0xFF63CA6C)),
                          ),
                        ),
                        onTap: () {
                          // 发送回复
  ...
  void sendReply(authorId) {
    ...
    if (replyStr.isEmpty || replyStr.trim().isEmpty) {
      return;
    } else {
      DataUtils.isLogin().then((isLogin) {
        if (isLogin) {
  ...
  Widget getTweetView(Map<String, dynamic> listItem) {
    ...
    var body = listItem['body'];
    body = clearHtmlContent(body);
    var contentRow = Row(
      children: <Widget>[
        Expanded(
          child: Text(body),
